...
class Tags {
	 ...
	 * @return void
	 */
	public static function post_meta( array $metas = array() ): void {
		$post_meta = (array) apply_filters( 'post_meta_show_on_post', $metas );

		// if nothing to show or explicitly disabled
		if ( empty( $post_meta ) || 'disabled' === ( $post_meta[0] ?? '' ) ) {
			return;
		}

		$classes = trim( 'list-unstyled d-flex entry-meta small text-muted ' . (string) self::$args['class'] );
