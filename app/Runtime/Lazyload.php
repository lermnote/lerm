...
class Lazyload {
	...
	protected $default_args = array(
		// �?HTML 中包含以下字符串时跳�?lazyload（可按需添加�?		'skip_list'    => array( 'skip_lazyload', 'custom-logo', 'slider', 'avatar', 'qrcode' ),
	...
	public function lazyload_match( $matches ) {
		...
		// 根据配置或属性决定是否跳�?		if ( $this->should_skip( $img_html ) ) {
			return $img_html;
		...
		// 跳过 data: URI（已经是内嵌的），或没有 src �?		if ( ! $src || stripos( $src, 'data:' ) === 0 ) {
			libxml_clear_errors();
			return $img_html;
		}

		// 根据 URL 的路径后缀判断是否为常见位�?图像格式；SVG 通常不需�?lazy
		...
		// �?src -> data-src，将 srcset -> data-srcset（如果存在），并把真�?src 替换�?1x1 占位透明像素
		...
		// 小巧透明占位�?x1 gif�?		$placeholder = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==';
		$img_node->setAttribute( 'src', $placeholder );

		// 输出修改后的 <img> 节点
		$modified_img = $doc->saveHTML( $img_node );

		// 可选地添加 <noscript> 回退，保留原�?HTML，保证无 JS 时能显示
		$final = $modified_img;
	...
	private function should_skip( $img_html ) {
		...
		// 跳过显式 data-skip 或类�?属性包�?skip_list 中的关键�?		foreach ( $skips as $needle ) {
			if ( stripos( $img_html, $needle ) !== false ) {
				return true;
			}
	

		// 如果标签里声明了 loading="eager" �?data-skip-lazy 属性，也跳�?		if ( preg_match( '/\bloading\s*=\s*([\'"]?)eager\1/i', $img_html ) ) {
			return true;
