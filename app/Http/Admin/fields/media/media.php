...
if ( ! class_exists( 'CSF_Field_media' ) ) {
	class CSF_Field_media extends CSF_Fields {
		...
		public function render() {
			...
			echo '<a href="#" class="button button-primary csf--button" data-library="' . esc_attr( $library ) . '" data-preview-size="' . esc_attr( $args['preview_size'] ) . '">' . wp_kses(
				$args['button_title'],
				array(
					'i'    => array( 'class' => array() ),
			...
			echo ( empty( $args['preview'] ) ) ? '<a href="#" class="button button-secondary csf-warning-primary csf--remove' . esc_attr( $hidden_auto ) . '">' . wp_kses(
				$args['remove_title'],
				array(
					'i'    => array( 'class' => array() ),
					'span' => array( 'class' => array() ),
