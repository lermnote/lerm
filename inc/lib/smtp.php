...
class SMTP {
	...
	public static function hooks() {
		...
		// add_action( 'wp_mail_failed', array( __CLASS__, 'wp_mail_debug' ), 10, 1 );

		add_action( 'comment_post', array( __CLASS__, 'comment_notify' ) );
		add_action( 'comment_unapproved_to_approved', array( __CLASS__, 'comment_notify' ) );
		add_action( 'comment_unapproved_to_approved', array( __CLASS__, 'comment_approved' ) );
	}
	...
	// Debug
	// public static function wp_mail_debug( $wp_error ) {
	// return error_log( print_r( $wp_error, true ) );
	// }

	public static function comment_approved( $comment ) {
	...
	public static function comment_notify( $comment_id ) {
		$comment        = get_comment( $comment_id );
		$parent_id      = $comment->comment_parent ? $comment->comment_parent : '';
		$spam_confirmed = $comment->comment_approved;
		if ( ( '' !== $parent_id ) && ( '1' === $spam_confirmed ) ) {
			$wp_email = kratos_option( 'm_username' );
