		...
		public function render() {
			$args = wp_parse_args(
				...
				array(
					...
					'preview'      => true,
					'library'      => array(),
					'button_title' => esc_html__( 'Upload', 'lerm' ),
					'remove_title' => esc_html__( 'Remove', 'lerm' ),
			...
			$placeholder = ( empty( $this->field['placeholder'] ) ) ? ' placeholder="' . esc_html__( 'No media selected', 'lerm' ) . '"' : '';

			echo $this->field_before();

			if ( ! empty( $args['preview'] ) ) {
				echo '<div class="csf--preview' . $hidden_auto . '">';
				echo '<div class="csf-image-preview"><a href="#" class="csf--remove fa fa-times">&times;</a><img src="' . $preview_src . '" class="csf--src" alt=""/></div>';
				echo '</div>';
			}
