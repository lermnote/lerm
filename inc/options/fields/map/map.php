	...
	class CSF_Field_map extends CSF_Fields {
		...
		public function render() {
			...
			$args = wp_parse_args(
				$this->field,
				array(
					'placeholder'    => esc_html__( 'Search...', 'lerm' ),
					'latitude_text'  => esc_html__( 'Latitude', 'lerm' ),
					'longitude_text' => esc_html__( 'Longitude', 'lerm' ),
					'address_field'  => '',
					'height'         => '',
				)
			);
		...
		public function enqueue() {
			...
			if ( ! wp_script_is( 'csf-leaflet' ) ) {
				wp_enqueue_script( 'csf-leaflet', esc_url( $this->cdn_url . $this->version . '/dist/leaflet.js' ), array( 'lerm' ), $this->version, true );
			}
