	...
	class CSF_Options extends CSF_Abstract {
		...
		public function set_options() {
			if ( wp_verify_nonce( csf_get_var( 'csf_options_nonce' ), 'csf_options_nonce' ) ) {
				...
				if ( ! empty( $transient['csf_import_data'] ) ) {
					...
					$this->notice = esc_html__( 'Success. Imported backup options.', 'lerm' );
					...
					$this->notice = esc_html__( 'Default options restored.', 'lerm' );
					...
					$this->notice = esc_html__( 'Default options restored for only this section.', 'lerm' );
				} else {
				...
				if ( empty( $this->notice ) ) {
					$this->notice = esc_html__( 'Settings saved.', 'lerm' );
				}
			}
		...
		public function add_admin_menu() {
			...
			if ( $menu_type === 'submenu' ) {
				$menu_page = call_user_func( 'add_theme_page', $menu_parent, $menu_title, $menu_title, $menu_capability, $menu_slug, array( &$this, 'add_options_html' ) );
			} else {
				$menu_page = call_user_func( 'add_theme_page', $menu_title, $menu_title, $menu_capability, $menu_slug, array( &$this, 'add_options_html' ), $menu_icon, $menu_position );
				...
				if ( ! empty( $this->args['show_sub_menu'] ) && count( $this->pre_tabs ) > 1 ) {
					...
					foreach ( $this->pre_tabs as $section ) {
						call_user_func( 'add_theme_page', $menu_slug, $section['title'], $section['title'], $menu_capability, $menu_slug . '#tab=' . $tab_key, '__return_null' );
		...
		public function add_options_html() {
			...
			echo ( $has_nav && $this->args['show_all_options'] ) ? '<div class="csf-expand-all" title="' . esc_html__( 'show all options', 'lerm' ) . '"><i class="fa fa-outdent"></i></div>' : '';

			echo ( $this->args['show_search'] ) ? '<div class="csf-search"><input type="text" placeholder="' . esc_html__( 'Search option(s)', 'lerm' ) . '" /></div>' : '';

			echo '<div class="csf-buttons">';
			echo '<input type="submit" name="' . $this->unique . '[_nonce][save]" class="button button-primary csf-save' . $ajax_class . '" value="' . esc_html__( 'Save', 'lerm' ) . '" data-save="' . esc_html__( 'Saving...', 'lerm' ) . '">';
			echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="' . esc_html__( 'Reset Section', 'lerm' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset this section options?', 'lerm' ) . '">' : '';
			echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button button-secondary csf-warning-primary csf-reset-all csf-confirm" value="' . esc_html__( 'Reset All', 'lerm' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset all options?', 'lerm' ) . '">' : '';
			echo '</div>';
			...
			if ( ! empty( $this->args['show_footer'] ) ) {
				...
				echo '<div class="csf-buttons">';
				echo '<input type="submit" name="csf_transient[save]" class="button button-primary csf-save' . $ajax_class . '" value="' . esc_html__( 'Save', 'lerm' ) . '" data-save="' . esc_html__( 'Saving...', 'lerm' ) . '">';
				echo ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="' . esc_html__( 'Reset Section', 'lerm' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset this section options?', 'lerm' ) . '">' : '';
				echo ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button button-secondary csf-warning-primary csf-reset-all csf-confirm" value="' . esc_html__( 'Reset All', 'lerm' ) . '" data-confirm="' . esc_html__( 'Are you sure to reset all options?', 'lerm' ) . '">' : '';
				echo '</div>';
