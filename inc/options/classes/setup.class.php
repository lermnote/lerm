...
if ( ! class_exists( 'CSF' ) ) {
	class CSF {
		...
		public static function add_admin_enqueue_scripts() {
			...
			// rtl styles
			if ( is_rtl() ) {
				wp_enqueue_style( 'csf-rtl', self::include_plugin_url( 'assets/css/csf-rtl' . $min . '.css' ), array(), '1.0.0', 'all' );
			}

			// framework core scripts
			wp_enqueue_script( 'csf-plugins', self::include_plugin_url( 'assets/js/csf-plugins' . $min . '.js' ), array(), '1.0.0', true );
			wp_enqueue_script( 'csf', self::include_plugin_url( 'assets/js/csf' . $min . '.js' ), array( 'csf-plugins' ), '1.0.0', true );
		...
		public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
			...
			if ( ! empty( $field['_notice'] ) ) {
				...
				$field            = array();
				$field['content'] = sprintf( esc_html__( 'Ooops! This field type (%s) can not be used here, yet.', 'lerm' ), '<strong>' . $field_type . '</strong>' );
				$field['type']    = 'notice';
				$field['style']   = 'danger';
			}
			...
			if ( ! empty( $field_type ) ) {
				...
				if ( class_exists( $classname ) ) {
					...
					echo '<p>' . esc_html__( 'This field class is not available!', 'lerm' ) . '</p>';
				}
			} else {
				echo '<p>' . esc_html__( 'This type is not found!', 'lerm' ) . '</p>';
			}
