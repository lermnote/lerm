...
if ( ! class_exists( 'CSF' ) ) {
	class CSF {
			...
			if ( ! empty( self::$args['shortcode_options'] ) ) {
				...
				foreach ( self::$args['shortcode_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						self::$shortcode_instances[ $key ] = CSF_Shortcoder::instance( $key, $params );
				...
				if ( ! empty( self::$shortcode_instances ) ) {
					foreach ( self::$shortcode_instances as $instance ) {
						if ( ! empty( $instance->args['show_in_editor'] ) ) {
		...
		public static function textdomain() {
			load_textdomain( 'lerm', self::$dir . '/languages/' . get_locale() . '.mo' );
		}
		...
		public static function add_admin_enqueue_scripts() {
			...
			if ( ! empty( self::$args['metaboxes'] ) ) {
				foreach ( self::$args['metaboxes'] as $argument ) {
			...
			if ( ! empty( self::$shortcode_instances ) ) {
				foreach ( self::$shortcode_instances as $argument ) {
					if ( $argument->args['show_in_editor'] && $wpscreen->base === 'post' ) {
						$enqueue = true;
					}
				}
			}
			...
			// Main style
			wp_enqueue_style( 'lerm', self::include_plugin_url( 'assets/css/style' . $min . '.css' ), array(), self::$version, 'all' );
			...
			wp_enqueue_script( 'lerm', self::include_plugin_url( 'assets/js/main' . $min . '.js' ), array( 'csf-plugins' ), self::$version, true );
			...
			wp_localize_script(
				...
				array(
					...
					'i18n'          => array(
						'confirm'         => esc_html__( 'Are you sure?', 'lerm' ),
						'typing_text'     => esc_html__( 'Please enter %s or more characters', 'lerm' ),
						'searching_text'  => esc_html__( 'Searching...', 'lerm' ),
						'no_results_text' => esc_html__( 'No results found.', 'lerm' ),
					),
				)
			);
		...
		public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
			...
			if ( ! empty( $field['_notice'] ) ) {
				...
				$field            = array();
				$field['content'] = esc_html__( 'Oops! Not allowed.', 'lerm' ) . ' <strong>(' . $field_type . ')</strong>';
				$field['type']    = 'notice';
				$field['style']   = 'danger';
			...
			if ( ! empty( $field_type ) ) {
				...
				if ( class_exists( $classname ) ) {
					...
					echo '<p>' . esc_html__( 'Field not found!', 'lerm' ) . '</p>';
				}
			} else {
				echo '<p>' . esc_html__( 'Field not found!', 'lerm' ) . '</p>';
			}
