    ...
    function csf_get_icons()
    {
        if (! empty($_POST['nonce']) && wp_verify_nonce($_POST['nonce'], 'csf_icon_nonce')) {
            ...
            if (! empty($icon_lists)) {
                ...
                echo '<div class="csf-text-error">'. esc_html__('No data provided by developer', 'lerm') .'</div>';
            }

            wp_send_json_success(array( 'success' => true, 'content' => ob_get_clean() ));
        } else {
            wp_send_json_error(array( 'success' => false, 'error' => esc_html__('Error while saving.', 'lerm'), 'debug' => $_REQUEST ));
        }
    }
        ...
        <div class="csf-modal-table-cell">
          ...
          <div class="csf-modal-inner">
            <div class="csf-modal-title">
              <?php esc_html_e('Add Icon', 'lerm'); ?>
              <div class="csf-modal-close csf-icon-close"></div>
            </div>
            <div class="csf-modal-header csf-text-center">
              <input type="text" placeholder="<?php esc_html_e('Search a Icon...', 'lerm'); ?>" class="csf-icon-search" />
            </div>
