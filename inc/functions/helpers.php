 ...
 * @since lerm 3.0
 */
function copyright_text( string $type = 'short' ) {
	$output   = '';
	$blogname = '<strong>' . esc_html( get_bloginfo( 'name' ) ) . '</strong>';
	...
	$first_post_date = get_posts(
		array(
			...
			'orderby'        => 'date',
			'order'          => 'ASC',
			'fields'         => 'post_date_gmt', // 只获取发布时间字段
		)
	);

	if ( ! empty( $first_post_date ) ) {
		$first_date = $first_post_date[0]->post_date_gmt;
	...
	$date         = ( $year === $current_year ) ? $current_year : $year . '-' . $current_year; // 合并年份计算
	if ( 'short' === $type ) {
