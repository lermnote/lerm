...
import { crossFadeReplace } from '../utils.js';
...
function renderPlaceholders(mount, count = 6) {
  // Ensure mount is the grid container; create row wrapper if mount isn't a row
  mount.innerHTML = '';
...
export async function fetchAndRenderPosts({ containerSelector = '#app', per_page = 6, embed = true, minSkeletonMs = 250 } = {}) {
  ...
  // ensure mount is using row grid classes (caller should set <div id="app" class="row g-4">)
...
export function initLoadMore(opts = {}) {
  ...
  const btn =document.querySelector(_pagerState.button) ;
