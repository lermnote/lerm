...
<div id="secondary" class="col-lg-4 d-none d-lg-block mb-3">
	<aside class="sidebar sidebar-affix">
		<?php if ( is_singular( 'post' ) ) : ?>
			...
			<?php if ( ! empty( $template_options['author_bio'] ) ) : ?>
		...
		<?php
		if ( is_home() ) :
			$blog_sidebar = $template_options['blog_sidebar_select'] ?: 'home-sidebar';
			dynamic_sidebar( $blog_sidebar );
		elseif ( is_front_page() ) :
			$front_page_sidebar = $template_options['front_page_sidebar'] ?: 'home-sidebar';
			dynamic_sidebar( $front_page_sidebar );
		endif;
		?>
		<?php
		if ( is_singular( 'page' ) ) :
			$page_sidebar = $template_options['page_sidebar'] ?: 'home-sidebar';
