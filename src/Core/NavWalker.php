...
use Walker_Nav_Menu;
if ( ! class_exists( __NAMESPACE__ . '\\NavWalker' ) ) :
	class NavWalker extends Walker_Nav_Menu {
		...
		public function add_schema_to_navbar_ul( $args ) {
			if ( isset( $args['items_wrap'] ) ) {
				...
				// Only proceed when the placeholder and an opening <ul exist and attribute not already present.
				if ( false === strpos( $wrap, 'SiteNavigationElement' ) && false !== strpos( $wrap, '%3$s' ) && false !== strpos( $wrap, '<ul' ) ) {
					// Inject itemscope/itemtype into the first <ul ...> tag.
					$args['items_wrap'] = preg_replace(
						'/<ul([^>]*)>/i',
						'<ul$1 itemscope itemtype="http://www.schema.org/SiteNavigationElement">',
						$wrap,
						1
					);
				}
