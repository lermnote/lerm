...
final class Image {
	...
	public static function get_image( array $params = array() ): array {
		...
		if ( $args['cache'] ) {
			$cached = wp_cache_get( "{$post_id}_thumbnail", 'post_thumbnail' );
			if ( $cached ) {
				return array(
					'id'  => (int) $cached,
					'src' => null,
		...
		foreach ( (array) $args['order'] as $order ) {
			if ( $attachment_id ) {
				break;
			}
			switch ( $order ) {
		...
		if ( $attachment_id ) {
			...
			if ( $args['cache'] ) {
				wp_cache_set( "{$post_id}_thumbnail", $attachment_id, 'post_thumbnail', HOUR_IN_SECONDS );
			}
		}
	...
	/** 从 Gutenberg blocks 里找 image block（返回 [id, src]） */
	...
	/** 扫描 content：优先 wp-image-ID，否则 img src（返回 [id, src]） */
	...
	/**
	 * 统一返回最终 URL（优先附件 URL，再 fallback 到 src）
	 *
	...
	public static function generate_image_html( array $result, array $args = array(), array $extra_attr = array() ): string {
		...
		// 默认 class 列表（可以被 args['class'] 或 args['classes'] 覆盖/扩展）
		$default_classes = array( 'attachment-' . $size, 'w-100', 'h-100', 'rounded' );

		// 处理 caller 传入的 class(s)
		...
		// 准备基本 attr（附件模式和 src fallback 共用）
		$base_attr = array(
			'class'   => $classes,
			'loading' => esc_attr( (string) $lazy ),
		);
		...
		if ( ! empty( $result['id'] ) && is_numeric( $result['id'] ) ) {
			...
			// wp_get_attachment_image 会自动输出 srcset 和 sizes（如果可用）
			return wp_get_attachment_image( $id, $size, false, $attr );
		}

		// 回退：只有 src（非附件或 attachment lookup 失败）
