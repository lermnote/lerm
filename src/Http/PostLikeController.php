...
use function Lerm\Functions\Utilities\client_ip;
 ...
 * @package Lerm\Inc\Rest
 */
final class PostLikeController extends BaseController {
	use Singleton;
	...
	public function create_item( $request ) {
		...
		// 1) 尝试解析 JSON body（最常见）
		$postdata = $request->get_json_params();

		// 2) 回退到常规 body params（表单 / x-www-form-urlencoded / multipart）
		if ( empty( $postdata ) ) {
	...
	public static function get_likes_button( $id, $is_comment = null, $args = array() ) {
		...
		$classes = array_filter( array( 'like-button', $args['class'], 'like-' . $type . '-' . $id, $liked ? 'btn-outline-danger' : 'btn-outline-secondary' ) );
		$label   = $liked ? __( 'Unlike', 'lerm' ) : __( 'Like', 'lerm' );

		$button = sprintf(
			'<button type="button" class="%1$s" aria-pressed="%2$s" aria-label="%3$s" data-id="%4$d" data-type="%5$s">%6$s <span class="count-wrap">%7$s</span></button>',
